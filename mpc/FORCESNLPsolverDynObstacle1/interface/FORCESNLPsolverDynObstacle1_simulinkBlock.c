...
#define S_FUNCTION_NAME FORCESNLPsolverDynObstacle1_simulinkBlock
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	/* Input Port 1 */
    ssSetInputPortMatrixDimensions(S,  1, 80, 1);
	...
	/* Input Port 2 */
    ssSetInputPortMatrixDimensions(S,  2, 144, 1);
    ...
    /* initialize output ports - there are 8 in total */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    /* Call solver */
    solver_exitflag = FORCESNLPsolverDynObstacle1_solve(&params, &output, &info, mem, fp , pt2function_FORCESNLPsolverDynObstacle1);
	...
	for(i = 0; i < 10; i++)
	{
		x6[i] = (real_T) output.x6[i];
	}

	for(i = 0; i < 10; i++)
	{
		x7[i] = (real_T) output.x7[i];
	}

	for(i = 0; i < 10; i++)
	{
		x8[i] = (real_T) output.x8[i];
	}
