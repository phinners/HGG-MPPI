...
#define S_FUNCTION_NAME FORCESNLPsolverDynObstacle1_simulinkBlockcompact
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	for(i = 0; i < 10; i++)
	{
		outputs[50 + i] = (real_T) output.x6[i];
	}

	for(i = 0; i < 10; i++)
	{
		outputs[60 + i] = (real_T) output.x7[i];
	}

	for(i = 0; i < 10; i++)
	{
		outputs[70 + i] = (real_T) output.x8[i];
	}
