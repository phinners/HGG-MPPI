...
% 
% [OUTPUTS] = FORCESNLPsolver1_1(INPUTS) solves an optimization problem where:
% Inputs:
% - xinit - matrix of size [6x1]
% - x0 - matrix of size [80x1]
% - all_parameters - matrix of size [144x1]
% Outputs:
...
function [x1, x2, x3, x4, x5, x6, x7, x8] = FORCESNLPsolver1_1(xinit, x0, all_parameters)
    
    [output, ~, ~] = FORCESNLPsolver1_1Buildable.forcesCall(xinit, x0, all_parameters);
    x1 = output.x1;
    x2 = output.x2;
    x3 = output.x3;
    x4 = output.x4;
