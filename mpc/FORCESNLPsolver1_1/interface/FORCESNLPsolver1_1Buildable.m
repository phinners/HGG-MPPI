...
% 
classdef FORCESNLPsolver1_1Buildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'FORCESNLPsolver1_1'))
                solverpath = fullfile(buildablepath, 'FORCESNLPsolver1_1');
            end
            solverInfo = struct();
            solverInfo.solvername = 'FORCESNLPsolver1_1';
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x6 = coder.nullcopy(zeros(10, 1));
            outputs_x7 = coder.nullcopy(zeros(10, 1));
            outputs_x8 = coder.nullcopy(zeros(10, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = FORCESNLPsolver1_1Buildable.forcesCall(params.xinit, params.x0, params.all_parameters);
        end

        function [output,exitflag,info] = forcesCall(xinit, x0, all_parameters)
            solvername = 'FORCESNLPsolver1_1';
            ...
            [output_c, exitflag_c, info_c] = FORCESNLPsolver1_1Buildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = FORCESNLPsolver1_1Buildable.forcesInitOutputsMatlab();
            ...
            output.x6 = cast(output_c.x6, 'like', output.x6);
            output.x7 = cast(output_c.x7, 'like', output.x7);
            output.x8 = cast(output_c.x8, 'like', output.x8);
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x6 = coder.nullcopy(double(zeros(10, 1)));
            outputs_x7 = coder.nullcopy(double(zeros(10, 1)));
            outputs_x8 = coder.nullcopy(double(zeros(10, 1)));
            output = struct('x1', outputs_x1,...
                            'x2', outputs_x2,...
                            'x3', outputs_x3,...
                            'x4', outputs_x4,...
