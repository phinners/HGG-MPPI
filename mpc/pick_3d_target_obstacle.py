...
def objective(z, p):
    u = z[0:4]
    x = z[4:10]
    acc_x_rel = u[0] / 30.0  # 30kg Weight
    acc_y_rel = u[1] / 30.0  # 30kg Weight
    acc_z_rel = u[2] / 30.0  # 30kg Weight

    return 390 * ((x[0] - p[0]) ** 2 + (x[1] - p[1]) ** 2 + (
            x[2] - p[2]) ** 2) + 0.01 * acc_x_rel ** 2 + 0.01 * acc_y_rel ** 2 + 0.01 * acc_z_rel ** 2 + 1000 * u[
        3] ** 2
...
def generate_pathplanner(create=True, path=''):
    ...
    if create:
        ...
        solver = forcespro.nlp.Solver.from_directory(path + "FORCESNLPsolver3DTargetObstacle1")
...
def main():
    ...
    args = get_args()
    args.env = 'FrankaFetchPick3DTargetObstacle-v1'
