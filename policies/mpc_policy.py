...
class MPCPolicy(Policy):
    ...
    def _predict(self, ob: np.ndarray, i: int) -> (np.ndarray, dict):
        ...
        xinit = np.transpose(
            np.array([grip_pos[0], grip_pos[1], grip_pos[2], self.agent_vel_x, self.agent_vel_y, self.agent_vel_z]))
        # xinit = np.transpose(
        #     np.array([grip_pos[0], grip_pos[1], grip_pos[2], grip_velp[0], grip_velp[1], grip_velp[2]]))
        ...
        problem = {'x0': x0, 'xinit': xinit, 'all_parameters': np.reshape(parameters, (model.npar * model.N, 1))}
    ...
    def _initial_info(self, ob: np.ndarray, env: MPCControlGoalEnv) -> dict:
        ...
        parameters = env.extract_parameters_3d(horizon=self.model.N, ts=self.integrator_ts, sub_goal=goal)
