...
def get_parameters(args):
    ...
    def state_cost(x, goal, obstacles):
        ...
        dist3 = x[:, 0:3] - obstacles[12:15]
        collision = torch.logical_or(torch.all(torch.le(dist1, torch.tensor(obstacles[3:6]) + 0.06), dim=1),
                                     torch.all(torch.le(dist2, torch.tensor(obstacles[9:12]) + 0.07), dim=1))
        collision = torch.logical_or(collision,
                                     torch.all(torch.le(dist3, torch.tensor(obstacles[15:18]) + 0.06), dim=1))
